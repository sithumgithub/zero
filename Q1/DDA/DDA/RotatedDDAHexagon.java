...
public class RotatedDDAHexagon extends JFrame {
    private int[] xPoints;
    private int[] yPoints;

    public RotatedDDAHexagon() {
        setTitle("Rotated DDA Hexagon Drawing");
        setSize(400, 400);
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setLocationRelativeTo(null);

        // Set the original coordinates of the hexagon vertices
        xPoints = new int[]{200, 300, 350, 300, 200, 150};
        yPoints = new int[]{100, 100, 200, 300, 300, 200};

        // Rotate the hexagon 90 degrees clockwise
        rotateClockwise90();
    }
    ...
    private void rotateClockwise90() {
        ...
        for (int i = 0; i < xPoints.length; i++) {
            // Use clockwise rotation matrix formula
            newXPoints[i] = yPoints[i];
            newYPoints[i] = 400 - xPoints[i];
        }
    ...
    private void drawRotatedDDAHexagon(Graphics g) {
    ...
    public void paint(Graphics g) {
        super.paint(g);
        drawRotatedDDAHexagon(g);
    }

    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            RotatedDDAHexagon rotatedHexagon = new RotatedDDAHexagon();
            rotatedHexagon.setVisible(true);
        });
    }
}
